...
if($totaldeleteid==""){
echo "<script>alert('Sorry No Record Found');window.location.href = './index.php?page=planingC';</script>";exit;
}
foreach($delete_ids as $key => $val)
  ...
  $dbConnection->exec($delete_que);
   ...
   echo "<script>alert('Record Delete Successfully');window.location.href = './index.php?page=planingC';</script>";exit;
...
for ($i=0; $i<$count; $i++) {
  $update_que="UPDATE `yearly_planC` SET indicator='$txtindicatore[$i]',unit='$txtunit[$i]',baseline='$txtbaseline[$i]',yeariyplan='$txtyeariyplan[$i]',quarterone='$txtquarterone[$i]',quartertwo='$txtquartertwo[$i]',quarterthree='$txtquarterthree[$i]',quarterfour='$txtquarterfour[$i]',rank='$txtrank[$i]',code='$txtcode[$i]' WHERE `id`='$iddds[$i]'"; 
   $dbConnection->exec($update_que);
}
echo "<script>alert('Update Successfully');window.location.href = './index.php?page=planingC';</script>";exit;
}
...
<tr>
    <td>
<h3><?= __('TOTAL EDIT IDS')?> "<?php echo $totaledit_ids; if($totaledit_ids==""){echo "<script>alert('Sorry No Record Found');window.location.href = 'planing.php';</script>";}?>"</h3>
...
<form action="./index.php?page=editeC" method="post" autocomplete="off">
<?php
$j = 0;
