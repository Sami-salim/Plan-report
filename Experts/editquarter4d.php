...
if($totaldeleteid==""){
echo "<script>alert('Sorry No Record Found');window.location.href = './index.php?page=Qfour_reportingd';</script>";exit;
}
foreach($delete_ids as $key => $val)
  ...
  echo "<script>alert('Delete Successfully');window.location.href = './index.php?page=Qfour_reportingd';</script>";exit;
 header("Location:Qfour_reportingd.php?msg=Record Delete Successfully");   
}
...
for ($i=0; $i<$count; $i++) {
  $update_que="UPDATE `yearly_pland` SET indicator='$txtindicatore[$i]',quarterfour_report='$txtquarterfour_report[$i]'  WHERE `id`='$iddds[$i]'"; 
    $dbConnection->exec($update_que);
}
echo "<script>alert('Update Successfully');window.location.href = './index.php?page=Qfour_reportingd';</script>";exit;
header("Location:Qfour_reportingd.php?msg=Update Successfully");  
}
...
<tr>
    <td>
<h3><?= __('TOTAL EDIT IDS')?> "<?php echo $totaledit_ids; if($totaledit_ids==""){echo "<script>alert('Sorry No Record Found');window.location.href = './index.php?page=Qfour_reportingd';</script>";}?>"</h3>
...
<form action="./index.php?page=editquarter4d" method="post" autocomplete="off">
<?php
$j = 0;
