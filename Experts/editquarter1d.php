...
if($totaldeleteid==""){
echo "<script>alert('Sorry No Record Found');window.location.href = './index.php?page=Qone_reportingd';</script>";exit;
}
foreach($delete_ids as $key => $val)
  ...
  $dbConnection->exec($delete_que);
   ...
   echo "<script>alert('Record Delete Successfully');window.location.href = './index.php?page=Qone_reportingd';</script>";exit;
...
for ($i=0; $i<$count; $i++) {
  $update_que="UPDATE `yearly_pland` SET indicator='$txtindicatore[$i]',quarterone_report='$txtquarterone_report[$i]'  WHERE `id`='$iddds[$i]'"; 
    $dbConnection->exec($update_que);
}  
 echo "<script>alert('Update Successfully');window.location.href = './index.php?page=Qone_reportingd';</script>";exit;
}
...
<head>
  <meta charset="utf-8">
   <title><?= __('Edit_Qone_reportC')?></title>
...
<tr>
    <td>
<h3><?= __('TOTAL EDIT IDS')?> "<?php echo $totaledit_ids; if($totaledit_ids==""){echo "<script>alert('Sorry No Record Found');window.location.href = './index.php?page=Qone_reportingd';</script>";}?>"</h3>
...
<form action="./index.php?page=editquarter1d" method="post" autocomplete="off">
<?php
$j = 0;
...
<th><?= __('quarterone report')?></th><td><input type="text" name="txtquarterone_report[]" value="<?=$row['quarterone_report'];?>" class="form-control" id="quarterone_report"></td></tr>	
   </td>
    <td align="right">
