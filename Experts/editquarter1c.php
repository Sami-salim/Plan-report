...
if($totaldeleteid==""){
echo "<script>alert('Sorry No Record Found');window.location.href = './index.php?page=Qone_reportingC';</script>";exit;
}
foreach($delete_ids as $key => $val)
  ...
  echo "<script>alert('Record Delete Successfully');window.location.href = './index.php?page=Qone_reportingC';</script>";exit;
...
for ($i=0; $i<$count; $i++) {
 ...
 echo "<script>alert('Update Successfully');window.location.href = './index.php?page=Qone_reportingC';</script>";exit;
}
...
<tr>
    <td>
<h3> <?= __('TOTAL EDIT IDS')?> "<?php echo $totaledit_ids; if($totaledit_ids==""){echo "<script>alert('Sorry No Record Found');window.location.href = './index.php?page=Qone_reportingC';</script>";}?>"</h3>
...
<form action="./index.php?page=editquarter1C" method="post" autocomplete="off">
<?php
$j = 0;
